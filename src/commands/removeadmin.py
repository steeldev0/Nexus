...
async def removeadmin(interaction, user_id: str):
    if interaction.user.id not in manager_ids:
        await interaction.response.send_message("You need to be a manager to use this command.", ephemeral=True)
        return

    try:
        user_id = int(user_id)
    except ValueError:
        await interaction.response.send_message("Please input a valid user ID.", ephemeral=True)
        return

    if user_id not in admin_ids:
        await interaction.response.send_message("This user isn't an admin.", ephemeral=True)
        return
